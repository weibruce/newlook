...
@Service
public class RefreshTokenScheduler implements SchedulerJobService {
    
    /* fields          ------------------------------------------------------*/

    private final static Logger logger = LoggerFactory.getLogger(RefreshTokenScheduler.class);
    ...
    @Override
    @Scheduled(fixedDelay = 2 * 59 * 60 * 1000, initialDelay = 5 * 1000)
    //@Scheduled(fixedDelay = 10 * 60 * 1000, initialDelay = 5 * 1000)
    public void runCronJob() {
        wechatIntegrationService.refreshGlobalAccessToken();
    }
}
